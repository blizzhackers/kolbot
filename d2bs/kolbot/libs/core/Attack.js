...
const Attack = {
	...
	clear: function (range, spectype, bossId, sortfunc, pickit = true) {
		...
		while (start && monsterList.length > 0 && attackCount < Config.MaxAttackCount) {
			...
			if (target && target.x !== undefined && (getDistance(target, orgx, orgy) <= range || (this.getScarinessLevel(target) > 7 && target.distance <= range))
				&& target.attackable) {
				Config.Dodge && me.hpPercent <= Config.DodgeHP && this.deploy(target, Config.DodgeRange, 5, 9);
				Misc.townCheck();
				tick = getTickCount();
	...
	storeStatistics: function (area) {
		...
		if (obj) {
			...
			obj[area].averageUniques = ((obj[area].averageUniques * obj[area].runs + Attack.uniques) / (obj[area].runs + 1)).toFixed(4);
			obj[area].runs += 1;

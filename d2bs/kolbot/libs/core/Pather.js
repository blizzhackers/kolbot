...
const Pather = {
	...
	moveNearPreset: function (area, unitType, unitId, minDist, clearPath = false, pop = false) {
		...
		if (!presetUnit) {
			throw new Error("moveNearPreset: Couldn't find preset unit - id: " + unitId + " unitType: " + unitType + " in area: " + getAreaName(area));
		}
	...
	moveToPreset: function (area, unitType, unitId, offX, offY, clearPath, pop) {
		...
		if (!presetUnit) {
			throw new Error("moveToPreset: Couldn't find preset unit - id: " + unitId + " unitType: " + unitType + " in area: " + getAreaName(area));
		}
	...
	moveToExit: function (targetArea, use, clearPath) {
		...
		console.info(true, "ÿc7MyArea: ÿc0" + getAreaName(me.area) + " ÿc7TargetArea: ÿc0" + getAreaName(targetArea));
		let areas = Array.isArray(targetArea) ? targetArea : [targetArea];
		let finalDest = areas.last();

		for (let i = 0; i < areas.length; i += 1) {
			if (me.area === areas[i]) {
				console.debug("Already in: " + getAreaName(areas[i]));
				continue;
			}
		...
		console.info(false, "ÿc7targetArea: ÿc0" + getAreaName(finalDest) + " ÿc7myArea: ÿc0" + getAreaName(me.area), "moveToExit");
		delay(300);
		...
		for (let i = 0; i < 5; i += 1) {
			...
			if (type === sdk.unittype.Object && unit.mode === sdk.objects.mode.Inactive) {
				if ((me.inArea(sdk.areas.Travincal) && targetArea === sdk.areas.DuranceofHateLvl1 && me.getQuest(sdk.quest.id.TheBlackenedTemple, sdk.quest.states.Completed) !== 1)
					...
					throw new Error("useUnit: Incomplete quest." + (!!targetArea ? " TargetArea: " + getAreaName(targetArea) : ""));
				}
	...
	useUnitEx: function (givenSettings = {}, targetArea = undefined) {
		...
		if (!unit) {
			throw new Error(
				...
				+ " MyArea: " + getAreaName(me.area) + (!!targetArea ? " TargetArea: " + getAreaName(targetArea) : "")
			);
		}

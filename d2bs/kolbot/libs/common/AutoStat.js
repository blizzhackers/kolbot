...
const AutoStat = new function () {
	...
	*/
	...
	this.getBlock = function () {
		...
		return Math.min(75, Math.floor((me.getStat(sdk.stats.ToBlock) + getBaseStat(15, me.classid, 23)) * (me.getStat(sdk.stats.Dexterity) - 15) / (me.charlvl * 2)));
	};
	...
	this.validItem = function (item) {
		...
		return me.getStat(sdk.stats.Strength) >= item.strreq && me.getStat(sdk.stats.Dexterity) >= item.dexreq && me.charlvl >= item.lvlreq;
	};
	...
	this.requiredDex = function () {
		...
		return Math.ceil((2 * me.charlvl * this.block) / (me.getStat(sdk.stats.ToBlock) + getBaseStat(15, me.classid, 23)) + 15) - me.getStat(sdk.stats.Dexterity) - inactiveDex;
	};
	...
	this.useStats = function (type, goal = false) {
		...
		while (getTickCount() - tick < 3000) {
			if (currStat > me.getStat(sdk.stats.StatPts)) {
				print("AutoStat: Using " + (currStat - me.getStat(sdk.stats.StatPts)) + " stat points in " + statIDToString[type]);
				return true;
			}
